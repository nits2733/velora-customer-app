...
// Native has no dev proxy — point straight at the deployed backend. Set via
// app.config.ts's extra.apiBaseUrl (or EXPO_PUBLIC_API_BASE_URL at build time).
const BASE_URL = (Constants.expoConfig?.extra?.apiBaseUrl as string) || 'https://api.velora.example.com'
...
export const api = {
  ...
  upload: <T>(path: string, file: RNUploadFile, folder?: string) => {
